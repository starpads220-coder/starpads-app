...
export function GooeyNav({
  ...
  const initialIndex = Math.max(0, items.findIndex((item) => pathname.startsWith(item.href)));
  ...
  useEffect(() => {
    ...
    const lis = navRef.current.querySelectorAll<HTMLLIElement>("li");
    ...
    const resizeObserver = new ResizeObserver(() => {
      const currentLis = navRef.current?.querySelectorAll<HTMLLIElement>("li");
      if (currentLis && currentLis[activeIndex]) {
        updateEffectPosition(currentLis[activeIndex]);
      }
    });
